        ...
        if(pdPASS == err)
        {
            ...
            switch (rev_msg.msg_type)
            {
				...
				case SYS_MSG_TYPE_NLP:
                {
                    ...
                    {
                        asr_result_callback_nlp("。", 3221, 1, 0, 0);
                    }
                    else if(rev_msg.msg_data.asr_data.nlp_data.flag == 2)
                    {
                        asr_result_callback_nlp("。", 3222, 1, 0, 0);
                    }
                    else if(rev_msg.msg_data.asr_data.nlp_data.flag == 3)
                    {
                        asr_result_callback_nlp("。", 3223, 1, 0, 0);
                    }
                    else
                    {
                        uint32_t semantic_id = 0;
                        int play_id_list[] = { -1, -1, -1 };
                        #if USE_SEPARATE_WAKEUP_EN
                        if (SYS_STATE_UNWAKEUP == get_wakeup_state())
                        {
                            cmd_handle_t cmd_handle = cmd_info_find_command_by_string(rev_msg.msg_data.asr_data.nlp_data.intent1);
                            if(cmd_info_is_wakeup_word(cmd_handle))
                            {
                                // mprintf("nlp wakeup  %s\n", rev_msg.msg_data.asr_data.nlp_data.intent1);
                        ...
                        {
                            set_state_enter_wakeup(EXIT_WAKEUP_TIME);
                            if(rev_msg.msg_data.asr_data.nlp_data.intent1[0] != '\0')
                            {
                                // mprintf("nlp result 1: %s\n", rev_msg.msg_data.asr_data.nlp_data.intent1);
                                cmd_handle_t cmd_handle_semantic = cmd_info_find_command_by_string(rev_msg.msg_data.asr_data.nlp_data.intent1);
                                semantic_id = cmd_info_get_semantic_id(cmd_handle_semantic);
                                play_id_list[0] = semantic_id;
                            }
                            if(rev_msg.msg_data.asr_data.nlp_data.intent2[0] != '\0')
                            {
                                // mprintf("nlp result 2: %s\n", rev_msg.msg_data.asr_data.nlp_data.intent2);
                                cmd_handle_t cmd_handle_semantic = cmd_info_find_command_by_string(rev_msg.msg_data.asr_data.nlp_data.intent2);
                                semantic_id = cmd_info_get_semantic_id(cmd_handle_semantic);
                                play_id_list[1] = semantic_id;
                            }
                            if(rev_msg.msg_data.asr_data.nlp_data.intent3[0] != '\0')
                            {
                                // mprintf("nlp result 3: %s\n", rev_msg.msg_data.asr_data.nlp_data.intent3);
                                cmd_handle_t cmd_handle_semantic = cmd_info_find_command_by_string(rev_msg.msg_data.asr_data.nlp_data.intent3);
                                semantic_id = cmd_info_get_semantic_id(cmd_handle_semantic);
                                play_id_list[2] = semantic_id;
                            }
                            if(rev_msg.msg_data.asr_data.nlp_data.intent4[0] != '\0')
                            {
                                // mprintf("nlp result 4: %s\n", rev_msg.msg_data.asr_data.nlp_data.intent4);
                                cmd_handle_t cmd_handle_semantic = cmd_info_find_command_by_string(rev_msg.msg_data.asr_data.nlp_data.intent4);
                                semantic_id = cmd_info_get_semantic_id(cmd_handle_semantic);
                                play_id_list[3] = semantic_id;
                            }
                            for(short i=0;i < 4;i++)
                            {
                                if (play_id_list[i] != -1)
                                {
                                    prompt_play_by_semantic_id(play_id_list[i],-1, default_play_done_callback,false);
                                }
                            }
                        }
                        #else
