        ...
        if(pdPASS == ret)
        {
            ...
            switch (rev_msg.msg_type)
            {
                ...
                {                    
                    ...
                    {
                        ret = network_send((int8_t *)data->payload,data->length);                        
                        // ret = network_send((int8_t *)data->payload+16, data->length-16);//pcm测试
                        // for(int i=0;i<(data->length-16);i++)                   
                        // {
                        //     mprintf("%.2X ", (unsigned char)*((int8_t *)data->payload+16+i));
                        // }
                    }
                        // mprintf("\n");
                    // toll_recv += data->length;
                    // toll_send += ret;
                    // ci_loginfo(LOG_USER,"Total_recv:(%d)this:(%d)_Total_send(%d)this(%d)\r\n",toll_recv,data->length,toll_send,ret);

                    break;
                } 
                default:
                {
                    // mprintf("msg not deal rev_msg.msg_type = 0x%x\n",rev_msg.msg_type);
                    break;
                }
            }
        }
